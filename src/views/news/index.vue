...
<!-- 表格start-->
    <el-row class="row-content" style="margin-top: 10px;min-height: 580px">
      <el-table :key='tables.tableKey' :data="tables.tableData" v-loading="tables.listLoading"
        ...
        <el-table-column prop="news_name" label="新闻名称" width="120"></el-table-column>
        <el-table-column prop="brief" label="简短介绍" show-overflow-tooltip></el-table-column>
        <el-table-column prop="type_status" label="新闻类型" width="100">
          <template slot-scope="scope">
            <div v-if="scope.row.type_status == 0">饮食</div>
            <div v-if="scope.row.type_status == 1">运动</div>
            <div v-if="scope.row.type_status == 2">健康</div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="type" label="新闻标签" show-overflow-tooltip></el-table-column>
        <el-table-column prop="desc" label="新闻详情" show-overflow-tooltip></el-table-column>
        <el-table-column prop="status" label="状态" width="80" align="center">
          <template slot-scope="scope">
            <el-tag
              :type="scope.row.status === 0 ? 'danger' : 'success'"
              disable-transitions>{{scope.row.status == 0? '隐藏':'显示'}}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog
      ...
      <el-form ref="dataForm" :inline="true" :model="formData"  label-position="right" label-width="100px"
        ...
        <el-form-item label="新闻名称" prop="news_name" >
          <el-input v-model="formData.news_name" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item label="新闻标签"  prop="type" >
          <el-input v-model="formData.type" style="width: 300px"></el-input>
        </el-form-item> <br>
        <el-form-item label="简短介绍" prop="brief"  >
          <el-input v-model="formData.brief" style="width: 715px"></el-input>
        </el-form-item>
        <br>

        <el-form-item label="新闻类型" prop="status">
          <el-select class="filter-item" v-model="formData.type_status" placeholder="新闻类型" style="width: 300px">
            <el-option :key="0" label="饮食" :value="0"></el-option>
            <el-option :key="1" label="运动" :value="1"></el-option>
            <el-option :key="2" label="健康" :value="2"></el-option>
          </el-select>
        </el-form-item>
...
<script>
  import {getData,getDataById,edit,add,del} from '@/api/news/news'
  export default {
    data() {
      return {
        tables: {
    ...
    methods: {
      ...
      edit(){
        let data = {
          id:this.formData.id,
          news_name:this.formData.news_name,
          brief:this.formData.brief,
          img:this.formData.img,
          type:this.formData.type,
          type_status:this.formData.type_status,
